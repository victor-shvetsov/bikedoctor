...
export const BLOCKS: Block[] = [
...

  {
    ...
    ownerNotes: "",
    files: [
      { path: "app/[slug]/page.tsx", status: "new", notes: "Dynamic route: fetch page_content by slug, resolve template, render" },
      { path: "app/[slug]/layout.tsx", status: "new", notes: "Shared public layout with navbar + footer" },
      { path: "lib/templates/template-registry.ts", status: "new", notes: "Maps template_type to React component" },
      { path: "lib/schemas/json-ld.ts", status: "new", notes: "JSON-LD generators per schema type" },
      { path: "components/templates/homepage-template.tsx", status: "new", notes: "Homepage template shell (sections filled in Block 1.1)" },
      { path: "components/templates/location-template.tsx", status: "new", notes: "Location page template shell" },
      { path: "components/templates/bike-type-template.tsx", status: "new", notes: "Bike type page template shell" },
      { path: "components/templates/brand-template.tsx", status: "new", notes: "Brand page template shell" },
    ],
    acceptanceCriteria: [
      { id: "0.3-a", description: "Any slug from page_content resolves to the correct template", met: false },
      { id: "0.3-b", description: "generateMetadata returns correct title, description, og tags from DB", met: false },
      { id: "0.3-c", description: "JSON-LD schema rendered in <head> per page's schema_types", met: false },
      { id: "0.3-d", description: "404 for unknown slugs", met: false },
      { id: "0.3-e", description: "Template shells render h1, subheadline, CTA from page_content", met: false },
      { id: "0.3-f", description: "Cross-links section renders links from cross_link_slugs", met: false },
      { id: "0.3-g", description: "generateStaticParams pre-generates all published slugs at build time", met: false },
    ],
    dependsOn: ["0.2"],
  },
